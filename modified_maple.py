...
class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self):
        ...
        visual_deep_prompts = [0 for i in range(len(self.compound_prompt_projections)-1)]
        compound_prompts_text=[0 for i in range(len(self.compound_prompt_projections)-1)]
        for index, layer in enumerate(self.compound_prompt_projections):
            if index % 2 == 0:
                visual_prompt = layer(self.compound_prompts_text_parameters[index])
                visual_deep_prompts[index] = visual_prompt
                self.print_info(visual_prompt, f"visual_deep_prompts[{index}]")
            else:
                text_prompt=layer(self.visual_deep_prompts_parameters[index])
                compound_prompts_text[index]=text_prompt
                ...
                self.print_info(compound_prompts_text, f"compound_prompts_text[{index}]")

        projected_ctx = self.proj_lang_to_vis(self.ctx)
        self.print_info(projected_ctx, "Projected ctx")

        return prompts, projected_ctx, compound_prompts_text, visual_deep_prompts
